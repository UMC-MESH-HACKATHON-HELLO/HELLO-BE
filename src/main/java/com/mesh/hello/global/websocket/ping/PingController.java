 ...
 * 연결 계층 검증용 최소 핑/에코.
 *
 * <p>클라이언트가 {@code /api/v1/ping}으로 SEND하면, 호출자에게만
 * {@code /user/api/v1/queue/pong}으로 본인 sessionId를 에코한다.
...
public class PingController {
    ...
    /**
     * {@link SendToUser}는 결과를 {@code /user/{principalName}/api/v1/queue/pong}으로 보낸다.
     * principalName = sessionId 이므로 호출자 본인에게만 도달한다.
     */
    @MessageMapping("/ping")
    @SendToUser("/api/v1/queue/pong")
